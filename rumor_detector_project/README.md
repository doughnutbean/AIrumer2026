...
3. 在从头运行流程中作为 `build_final_fusion_predictions.py` 的本地主干输入。

此外，项目补充了 `train_dedicated_local.py` 和 `dedicated_model.py`，用于从 `train.csv` 重新训练一个本地 TF-IDF 多视角集成模型。该训练模型可作为 `models/dedicated_local.joblib` 保存，并可通过 `hybrid_classifier.py` 与原有 `predict.py` 统一加载。
...
```text
accuracy = 0.9002493766
confusion_matrix = [[216, 10], [30, 145]]
```

这说明：

```text
90% 左右在候选层面是有机会的，关键在于真实复核器能否高精度识别这些 FN。
```
...
### 7.6 从头生成 final-fusion 的结果说明

项目新增 `build_final_fusion_predictions.py` 后，可以从当前本地模型、Transformer 输出和检索器重新生成 `outputs/final_fusion_with_transformer/predictions.jsonl`。在当前保存的 `models/ensemble.joblib` 和重新构建的 `retriever.joblib` 条件下，从头生成的本地 final-fusion 结果约为：

```text
accuracy = 0.8827930175
confusion_matrix = [[214, 12], [35, 140]]
```

该结果低于仓库中保留的实验阶段强主干结果。由于从头训练过程涉及模型初始化、Transformer 微调、阈值选择、融合策略和 LLM 复核输出，重新运行可能与最终保存结果 `0.8902743142` 产生一定出入。因此，课程复核建议以仓库中最初保留的 `outputs/final_fusion_with_transformer/predictions.jsonl` 作为 FN-aware selective correction 的输入依据，并以 `outputs/fn_aware_selective_correction/predictions.jsonl` 作为最终结果复核文件。
...
├── models/
│   ├── ensemble.joblib
│   ├── dedicated_local.joblib              # 可由 train_dedicated_local.py 生成
│   ├── retriever.joblib
│   └── transformer_voter/                  # 可选，Transformer voter 训练输出
│       └── manifest.json
├── outputs/
...
│   ├── transformer_voter_val.jsonl         # 可选，Transformer voter 概率导出
...
└── src/
    ...
    ├── retriever.py
    ├── dedicated_model.py
    ├── train_dedicated_local.py
    ├── train_transformer_voter.py
    ├── predict_transformer_voter.py
    ├── export_transformer_voter_predictions.py
    ├── build_final_fusion_predictions.py
    ├── llm_client.py
    ├── llm_reranker.py
...
其中，`dataset/` 目录需要放置课程提供的 `train.csv` 和 `val.csv`。如果 GitHub 仓库不直接包含数据集，应在运行前手动创建目录并放入对应文件。
...
Transformer voter 训练为可选功能，若需要重新训练 Transformer，则还需要：
...
如果需要重新训练 Transformer voter，可额外安装：
...
### 13.5 从头训练与生成 final-fusion JSONL

本项目补充了从数据重新训练本地模型、重新构建检索器、导出 Transformer voter 概率并生成 final-fusion JSONL 的脚本。完整流程如下。

第一步，准备数据：

```bash
mkdir -p dataset
cp train.csv dataset/train.csv
cp val.csv dataset/val.csv
```

第二步，训练本地辅助模型。快速复现模式如下：
...
第五步，融合本地模型、Transformer 输出和检索特征，生成 final-fusion JSONL：
...
如果没有 `dedicated_local.joblib` 或 `outputs/transformer_voter_val.jsonl`，该脚本仍可运行，并会自动使用可用模型与检索特征。生成的 `predictions.jsonl` 可继续作为 `fn_aware_selective_correction.py` 的输入。

第六步，基于新生成的 final-fusion 结果运行 FN-aware selective correction：
...
python src/fn_aware_selective_correction.py \
  ...
  --out-dir outputs/fn_aware_selective_correction
```

如果需要真实 LLM 复核，在上述命令中加入第 13.3 节的 LLM 参数。

从头训练结果可能与最终保存结果存在差异。差异来源包括本地模型训练随机性、Transformer 微调随机性、阈值搜索、检索器重建和 LLM 输出不稳定性。因此，最终成绩复核建议以仓库中最初保留的 `outputs/final_fusion_with_transformer/predictions.jsonl` 为输入依据，并以 `outputs/fn_aware_selective_correction/predictions.jsonl` 作为最终提交结果文件。
...
`build_final_fusion_predictions.py` 会生成：

```text
outputs/final_fusion_with_transformer/
├── metrics.json
└── predictions.jsonl
```

其中 `predictions.jsonl` 包含本地概率、Transformer 概率、检索近邻、FN-risk 辅助字段和基础预测标签，可作为选择性复核脚本的输入。
...
检索结果会提供给 DeepSeek 作为 few-shot 证据，也会写入 `build_final_fusion_predictions.py` 生成的 final-fusion JSONL。

### 15.3 `src/dedicated_model.py`

定义本地辅助模型的可复用组件，包括：

- `MultiTfidfVectorizer`：融合 word-level、char-level 和 hashtag-oriented TF-IDF 特征；
- `WeightedProbabilityEnsemble`：对多个概率分类器进行加权融合，并支持阈值化预测。

该文件用于保证 `train_dedicated_local.py` 训练出的 `dedicated_local.joblib` 能够被后续预测和融合脚本稳定加载。

### 15.4 `src/train_dedicated_local.py`

从 `dataset/train.csv` 和 `dataset/val.csv` 训练本地 TF-IDF 多视角集成模型。脚本支持：

- 多组 Logistic Regression、SGD、Naive Bayes 和 SVM 候选模型；
- 验证集阈值搜索；
- `--quick` 快速训练模式；
- 输出 `models/dedicated_local.joblib` 和 `outputs/dedicated_local_metrics.json`。

该脚本用于补充本地主干模型的训练过程，使项目不只依赖预保存模型文件。

### 15.5 `src/train_transformer_voter.py`

用于微调 Transformer voter，例如 `vinai/bertweet-base`。脚本支持多 seed 训练、验证集评估和 manifest 文件保存。该模块为语义主干信号提供可复训练入口，属于可选训练组件。

### 15.6 `src/predict_transformer_voter.py`

加载 `models/transformer_voter/manifest.json` 中记录的多个 seed 模型，对输入 CSV 生成 Transformer 概率预测，并导出 JSONL 结果。该脚本主要用于单独分析 Transformer voter 的预测行为。

### 15.7 `src/export_transformer_voter_predictions.py`

将 Transformer voter 的平均概率导出为：

```text
outputs/transformer_voter_val.jsonl
```

该文件可作为 `build_final_fusion_predictions.py` 的可选输入。默认不覆盖 `outputs/final_fusion_with_transformer/predictions.jsonl`，以避免破坏保存的最终主干结果。

### 15.8 `src/build_final_fusion_predictions.py`

生成 final-fusion prediction JSONL，是本次补充的核心脚本。该脚本负责：

- 读取 `val.csv`；
- 加载 `models/ensemble.joblib` 获取本地基础概率；
- 可选加载 `models/dedicated_local.joblib` 获取辅助概率；
- 可选读取 `outputs/transformer_voter_val.jsonl` 获取 Transformer 概率；
- 加载 `models/retriever.joblib` 生成 Top-K 相似样本、Top-1 标签、相似度和 KNN 谣言比例；
- 计算 suspicious score 和 evidence 字段；
- 写出 `outputs/final_fusion_with_transformer/predictions.jsonl` 和 `metrics.json`。

该脚本补齐了从本地模型、Transformer 输出和检索特征到 FN-aware selective correction 输入文件之间的工程缺口。

### 15.9 `src/llm_client.py`
...
### 15.11 `src/hybrid_classifier.py`

单条文本预测入口的核心分类器，负责：

- 加载本地 `ensemble.joblib` 模型；
- 兼容加载 `dedicated_local.joblib` 模型；
- 输出本地预测概率；
- 提取可解释词项特征；
- 在需要时调用 DeepSeek 复核。

该文件已进行兼容性修改，使原始 AIrumer 模型和新增训练脚本生成的模型可以通过同一预测接口调用。

### 15.12 `src/fn_aware_selective_correction.py`

最终评测脚本，负责：

- 读取主干预测结果；
- 兼容 `base_label/base_prob_1` 与 `transformer_label/transformer_prob_1` 两类输入格式；
- 构造 FN-risk score；
- 排序候选样本；
- 调用 DeepSeek；
- 应用高置信翻转门控；
...
该文件已进行输入兼容性修改，使其既可读取保存的 final-fusion 结果，也可读取重新生成的简化 Transformer JSONL 或融合 JSONL。

### 15.13 `src/predict.py`

面向单条文本的命令行预测入口，负责调用 `hybrid_classifier.py` 输出分类结果与解释字段。该脚本用于满足“输入一个文本，输出检测和判断依据”的基本功能要求。
...
6. **流程更完整**：补充了本地训练、Transformer 训练、概率导出和 final-fusion JSONL 生成脚本。

### 16.2 局限

1. 真实 DeepSeek 复核尚未稳定超过 90%；
2. LLM 输出可能受 prompt、接口状态和模型版本影响；
3. 高相似度阈值和客观新闻保护规则需要根据数据集校准；
4. 如果换到新事件、新平台或新语言，需要重新评估 FN-risk 分布；
5. 当前保存的 89.03% 最终结果依赖实验阶段保留的 final-fusion 主干输出和 LLM 复核结果；
6. 从头训练和重新生成 final-fusion 可能与保存结果存在一定出入，因此最终成绩复核建议以仓库保留的原始 final-fusion 文件为依据。
...
# 3. 构建检索器，如果 models/retriever.joblib 不存在
python src/retriever.py --train dataset/train.csv --out models/retriever.joblib

# 4. 复核仓库中保存的最终结果
python src/fn_aware_selective_correction.py --top-k 20 --review-threshold 0.55

# 5. 运行 oracle 上界模拟
python src/fn_aware_selective_correction.py --simulate-oracle --top-k 20 --review-threshold 0.55

# 6. 运行真实 DeepSeek 复核评测
python src/fn_aware_selective_correction.py --use-llm --top-k 20 --review-threshold 0.55 --llm-confidence-min 0.80 --min-top1-similarity 0.35 --min-knn5-rumor-ratio 0.20 --strict-objective-protection --llm-sleep-seconds 7
```

如需从头训练本地模型并重新生成 final-fusion 文件，可执行第 13.5 节的完整流程。该流程主要用于验证工程完整性，不保证重新得到与保存结果完全一致的 89.03%。
...
项目同时补充了本地模型训练、Transformer voter 训练与导出、final-fusion JSONL 生成和选择性复核脚本，形成了较完整的工程实践流程。由于从头训练可能受到模型随机性、融合策略和 LLM 输出稳定性的影响，最终结果复核建议以仓库中保留的原始 `outputs/final_fusion_with_transformer/predictions.jsonl` 和 `outputs/fn_aware_selective_correction/predictions.jsonl` 为依据。
